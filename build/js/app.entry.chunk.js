...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //兼容性库 。es7
...
//ES6 学习
////import esjs from './js/estest.js' 
////import esjs from './js/fntest.js'
////import esjs from './js/promise.js'
//import esjs from './js/es6gaoji.js'

//var tmp=html
//function a(){
//var dom = document.getElementsByClassName('child')
////console.log(tmp)
////console.log(para)
//}
//
...
//import lotterystyle from './css/lottery.css' 

__webpack_require__(125);

var _lottery = __webpack_require__(327);
...
var test = function () {
  ...
  _createClass(test, [{
    ...
    value: function f() {
      var self = this;
      console.log(this.list.get(r).tip);
    }
  }]);
...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //数据请求
...
var interfacecode = function () {
  function interfacecode() {
    _classCallCheck(this, interfacecode);
  }

  _createClass(interfacecode, [{
    key: 'getomit',
...
var timer = function () {
  ...
  _createClass(timer, [{
    key: 'countdown',
    value: function countdown(end, updata, handle) {
      //开始时间，时间结束后更新，倒计时结束后回调
      var now = new Date().getTime();
      var self = this;
      if (end > now) {
        handle.call(self);
      } else {
        var lasttime = end - now; //剩余时间
        var px_d = 1000 * 60 * 60 * 24;
        var px_h = 1000 * 24 * 24;
        var px_m = 1000 * 60;
        var px_s = 1000;
        var d = Math.floor(lasttime / px_d); //剩余天数
        var h = Math.floor((lasttime - d * px_d) / px_h);
        var m = Math.floor((lasttime - d * px_d - h * px_h) / px_m);
        var s = Math.floor((lasttime - d * px_d - h * px_h - m * px_m) / px_s);
        var r = [];
        if (d > 0) {
          r.push('<em>' + d + '</em>\u5929');
        }
